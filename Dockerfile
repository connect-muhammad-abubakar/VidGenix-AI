...
# 设置/VidGenix AI目录权限为777
RUN chmod 777 /VidGenix AI

ENV PYTHONPATH="/VidGenix AI"
...
## For Linux or MacOS:
# docker run -v $(pwd)/config.toml:/VidGenix AI/config.toml -v $(pwd)/storage:/VidGenix AI/storage -p 8501:8501 moneyprinterturbo
## For Windows:
# docker run -v ${PWD}/config.toml:/VidGenix AI/config.toml -v ${PWD}/storage:/VidGenix AI/storage -p 8501:8501 moneyprinterturbo